...
window.addEventListener("DOMContentLoaded", function () {
  ...
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])(".popup-design", ".button-design");
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])(".popup-consultation", ".button-consultation");
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])(".popup-gift", ".fixed-gift", true);
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["showModalByTime"])(".popup-consultation", 5000);
...
var accordion = function accordion(btnsSelector) {
...
var burger = function burger(burgerSelector, menuSelector) {
  ...
  window.addEventListener("resize", function () {
    //Если человек разворачивает планшет и ширина становится больше, то меню скрывается.
...
var calc = function calc(size, material, service, promocode, result) {
  ...
  var getDataPromocode = function getDataPromocode(e) {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResourse"])('http://localhost:3000/promocode').then(function (res) {
      return calcFunc(e, res);
    }).catch(function (error) {
      return console.log(error);
    });
  };

  var getDataPrice = function getDataPrice(e) {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResourse"])('http://localhost:3000/price').then(function (res) {
...
var drop = function drop() {
  ...
  var highlite = function highlite(input) {
...
var filter = function filter() {
  ...
  var filterPictures = function filterPictures(picturesSelector) {
    var pictures = document.querySelectorAll(picturesSelector);
    console.log(pictures);
    pictureAll.forEach(function (picture) {
  ...
  var activeBtn = function activeBtn(e) {
    var target = e.target;
    ...
    if (target && target.tagName == 'LI') {
      ...
      filterPictures(".portfolio-block.".concat(target.className.split(' ')[0]));
    }
...
var forms = function forms(formSelector) {
  ...
  var addNameFile = function addNameFile() {
    upload.forEach(function (item) {
  ...
  var bindPostData = function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      ...
      var api = form.closest('.popup-design') || form.classList.contains('calc_form') ? path.designer : path.question;
      console.log(api);
...
/*!**********************************!*\
  !*** ./src/js/modules/modals.js ***!
  \**********************************/
/*! exports provided: default, showModalByTime, openByScroll */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var openByScroll = function openByScroll(selectorModal, triggerSelector) {
...
var pictureSize = function pictureSize(pictureSelector) {
  var pictures = document.querySelectorAll(pictureSelector);

  var showPicture = function showPicture(picture) {
    var img = picture.querySelector('img'),
        dataImg = picture.querySelectorAll('p:not(.sizes-hit)');
    img.src = img.src.slice(0, -4) + '-1.png'; //img.png => img-1.png

    dataImg.forEach(function (data) {
      return data.style.display = 'none';
    });
  };

  var hidePicture = function hidePicture(picture) {
    var img = picture.querySelector('img'),
        dataImg = picture.querySelectorAll('p:not(.sizes-hit)');
...
var scrolling = function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      function step(time) {
        //На следующую итерацию(запуск) анимации, переменная start null не будет являться
        if (start === null) {
          //Время прошедшее с момента начала загрузки страницы
          start = time;
        } //Cколько уже пролистано пикселей


        var progress = time - start,
            //Cколько нужно пикселей пролистать в течениии этой анимации
        //пролистать вверх(всего(px) => 0) : пролистать вниз(0 => всего(px)) 
...
var showMoreStyles = function showMoreStyles(trigger, stylesWrapper) {
  ...
  btn.addEventListener("click", function () {
    ...
    }) //res.styles если мы напрямую обращаемся к файлу без сервера
    .catch(function (error) {
      return console.log(error);
...
var sliders = function sliders(slideSelector, dir, prevSelector, nextSelector) {
  ...
  var shiftSlide = function shiftSlide() {
    try {
